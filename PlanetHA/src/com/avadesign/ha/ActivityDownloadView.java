		...
		protected Boolean doInBackground(String... params) 
		{
			...
			if (!params[3].equalsIgnoreCase("location"))
			{
				bm_on=SendHttpCommand.GetCamImage(
						String.format( cp.isLocalUsed() ? getString(R.string.local_url_syntax) : getString(R.string.server_url_syntax) , cp.isLocalUsed() ? cp.getControllerIP() : getString(R.string.server_ip) , cp.isLocalUsed() ? String.valueOf(cp.getControllerPort()) : getString(R.string.server_port) )+params[1]+(cp.isLocalUsed()?"":"?mac="+cp.getControllerMAC()+"&username="+cp.getControllerAcc()+"&userpwd="+cp.getControllerPwd()+"&tunnelid=0"),
						cp.getControllerAcc(),
						cp.getControllerPwd(),
						50);
				
				bm_off=SendHttpCommand.GetCamImage(
						String.format( cp.isLocalUsed() ? getString(R.string.local_url_syntax) : getString(R.string.server_url_syntax) , cp.isLocalUsed() ? cp.getControllerIP() : getString(R.string.server_ip) , cp.isLocalUsed() ? String.valueOf(cp.getControllerPort()) : getString(R.string.server_port) )+params[2]+(cp.isLocalUsed()?"":"?mac="+cp.getControllerMAC()+"&username="+cp.getControllerAcc()+"&userpwd="+cp.getControllerPwd()+"&tunnelid=0"),
						cp.getControllerAcc(),
						cp.getControllerPwd(),
						50);
		...
		protected void onPostExecute(final Boolean success) 
		{
			...
			if (bm_normal!=null)
			{
				...
				if (ary.length>=3)
				{
					...
					String path= Environment.getExternalStorageDirectory().toString()+"/Android/data/com.avadesign.ha"+"/"+ary[0]+"/"+ary[1]+(group.equalsIgnoreCase("location") ? "" : "/normal");
			...
			if(!group.equalsIgnoreCase("location"))
			{
				if (bm_on!=null)
				{
					...
					if (ary.length>=3)
					{
						String path= Environment.getExternalStorageDirectory().toString()+"/Android/data/com.avadesign.ha"+"/"+ary[0]+"/"+ary[1]+(group.equalsIgnoreCase("location") ? "" : "/on");
				...
				if (bm_off!=null)
				{
					...
					if (ary.length>=3)
					{
						String path= Environment.getExternalStorageDirectory().toString()+"/Android/data/com.avadesign.ha"+"/"+ary[0]+"/"+ary[1]+(group.equalsIgnoreCase("location") ? "" : "/off");
