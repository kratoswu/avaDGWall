...
import com.avadesign.AvaCallActivity;
...
//		Intent notifIntent = new Intent(this, LinphoneActivity.class);
	    Intent notifIntent = new Intent(this, AvaCallActivity.class);
	...
	private Class<? extends Activity> incomingReceivedActivity = AvaCallActivity.class;
...
//				}
	...
	private Map<String, JSONObject> getDPMap() {
	    Map<String, JSONObject> result = new HashMap<String, JSONObject>();
	    
	    try {
	        AvaPref pref = ((SharedClassApp) getApplication()).getAppPref();
	        Set<String> dpStrSet = pref.getValueSet(getString(R.string.key_dplist));
	        
	        for (String s : dpStrSet) {
	            JSONObject jo = new JSONObject(s);
	            result.put(jo.getString(getString(R.string.key_dp_sip)), jo);
	        }
	    } catch (Exception e) {
	...
	public void onCallStateChanged(final LinphoneCall call, final State state, final String message) {
		...
		if (state == LinphoneCall.State.IncomingReceived) {
		    /*
		     * TODO
		     * address format: sip:2001@192.168.1.104
		     * 判斷邏輯改成先比對 dp list, 沒有的話再轉成 avacall.
		     * */
            android.util.Log.e("Remote call info:", call.getRemoteAddress().asString());
            String callAddr = call.getRemoteAddress().asString();
            boolean isSip = callAddr.contains("@");
            
            if (isSip) {
                String sipID = callAddr.substring(callAddr.indexOf(":") + 1, callAddr.indexOf("@"));
                isSip = !getDPMap().containsKey(sipID);
            }
            
           incomingReceivedActivity = isSip ? AvaCallActivity.class : DoorActivity.class;
           LinphonePreferences mPrefs = LinphonePreferences.instance();
           mPrefs.enableVideo(isSip);
           mPrefs.setInitiateVideoCall(isSip);
           mPrefs.setAutomaticallyAcceptVideoRequests(isSip);
